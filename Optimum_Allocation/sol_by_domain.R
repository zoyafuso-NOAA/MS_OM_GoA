...
winner = which.min(sample_sizes[settings$cv == 0.15])
ns = 15
goa = SpatialPointsDataFrame(coords = Extrapolation_depths[,c('E_km', 'N_km')], 
                             data = data.frame(X1=res_df[,paste0('V',
                                                                 winner+2)]) )
...
{tiff(paste0('model_', VAST_model, '/solution_space.tiff'), 
      res = 200, width = 5, height = 3.65, units = 'in', compression = 'lzw')
