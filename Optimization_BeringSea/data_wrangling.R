...
for(id in sort(unique(df$STATIONID)[unique(df$STATIONID) != 'J-13'] )){
  ...
  mutinfo <- multiinformation(discretize(temp_df))
...
plot(lat ~ long, data = df2, cex = tot_mean^2 * 10, axes = F); box()
plot(lat ~ long, data = df2, cex = tot_var^2 * 100, axes = F); box()
