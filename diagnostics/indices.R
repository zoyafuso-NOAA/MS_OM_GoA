...
## Description:   Plot mean and 1 SD of abundance indices
###############################################################################
rm(list = ls())
...
{
  png(paste0(diag_dir, "indices_comparison_DBE.png"), 
      width = 12, 
      height = 6, 
      units = "in", 
      res = 500)
  ...
  for (spp in spp_to_include) {
    ...
    #Empty plot
    plot(x = Year_Set[Years2Include], 
         y = Index_Ests[spp,] / 1e6, 
         type = "n", 
         ylab = "Index", 
         xlab = "Year", 
         las = 1, 
    ...
    polygon(x = c(Year_Set[Years2Include],
            ...
            y = c(lower[spp,], rev(upper[spp,])), 
            col = "grey", 
            lty = "dotted")
    
    lines(x = Year_Set[Years2Include], 
          y = Index_Ests[spp,] / 1e6)
    points(x = Year_Set[Years2Include], 
           y = Index_Ests[spp,] / 1e6, 
           pch = 16)
    
    #Plot design-based intervals
    points(Year_Set[Years2Include],
           temp_DBE$TOTAL_BIOMASS / 1e6, 
           col = "red", 
           pch = 16)
    ...
    mtext(side = 3, 
          text = unique(temp_DBE$COMMON_NAME), 
          font = 1)
  ...
  mtext(side = 2, 
        outer = T, 
        text = "Abundance Index (million metric tons)",
        line = 1)
  dev.off()
}
