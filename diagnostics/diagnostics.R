##Diagnostic plots
## We first apply a set of standard model diagnostics to confirm that the model is 
...
## Plot data: It is always good practice to conduct exploratory analysis of data.  
## Here, I visualize the spatial distribution of data.  Spatio-temporal models involve 
## the assumption that the probability of sampling a given location is statistically 
## independent of the probability distribution for the response at that location.  
...
## Convergence: Here I print the diagnostics generated during parameter estimation, 
## and I confirm that (1) no parameter is hitting an upper or lower bound and (2) 
## the final gradient for each fixed-effect is close to zero. For explanation of
...
## first determining which years to plot (`Years2Include`), and labels for each plotted 
## year (`Year_Set`)
...
## positive or negative residuals accross or within years) then this is an indication of 
## the model "overshrinking" results towards the intercept, and model results should then 
## be treated with caution.  
if(!dir.exists(paste0(DateFile, "Pearson_Residuals"))) dir.create(paste0(DateFile, "Pearson_Residuals/"))
...
## Plot spatial and spatio-temporal covariance: We can visualize the spatial and 
## spatio-temporal covariance among species in encounter probability and positive 
## catch rates (depending upon what is turned on via `FieldConfig`):
...
## Density surface for each year: We can visualize many types of output from the model.
