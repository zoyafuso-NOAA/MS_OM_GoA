fit_model = function (settings, Lat_i, Lon_i, t_iz, b_i, a_i, c_iz = rep(0, 
    length(b_i)), v_i = rep(0, length(b_i)), working_dir = paste0(getwd(), 
    "/"), Xconfig_zcp = NULL, covariate_data, formula = ~0, Q_ik = NULL, 
    newtonsteps = 1, silent = TRUE, build_model = TRUE, run_model = TRUE, 
    test_fit = TRUE, X_gtp = NULL, ...) 
{
    extra_args = list(...)
    ...
    data_frame = data.frame(Lat_i = Lat_i, Lon_i = Lon_i, a_i = a_i, 
        v_i = v_i, b_i = b_i, t_i = t_iz, c_iz = c_iz)
    ...
    data_args_default = list(Version = settings$Version, FieldConfig = settings$FieldConfig, 
        ...
        t_iz = t_iz, spatial_list = spatial_list, Options = settings$Options, 
    ...
    if (exists("check_fit") & test_fit == TRUE) {
        ...
        if (problem_found == TRUE) {
            message("\n")
            stop("Please change model structure to avoid problems with parameter estimates and then re-try; see details in 
`?check_fit`\n", 
                call. = FALSE)
        }
    }
